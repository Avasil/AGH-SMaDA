%Simulation with boundary conditions type 2 and heat dissipation
...
for t=1:Nt-1
    ...
    for i=2:Nx-1
        for j=2:Ny-1
            tempDiff=-5.678*10^-8*T(i,j,t)*T(i,j,t)*T(i,j,t)*T(i,j,t)*(T(i,j,t)-outsideTemp);
            if (~((i>=(0.075/dx) &&  i<=(Nx-0.075/dx)) && (j>=(0.075/dy) &&  j<=(Nx-0.075/dy) ) ) || t>=(1/dt) )
            T(i,j,t+1)=T(i,j,t)+(K*dt/Cw*Rho*dx*dx)*(T(i+1,j,t)-2*T(i,j,t)+T(i-1,j,t))+(K*dt/Cw*Rho*dy*dy)*(T(i,j+1,t)-2*T(i,j,t)+T(i,j-1,t))+tempDiff;
            else
               if t<(1/dt)
                T(i,j,t+1)=T(i,j,t)+(K*dt/Cw*Rho*dx*dx)*(T(i+1,j,t)-2*T(i,j,t)+T(i-1,j,t))+(K*dt/Cw*Rho*dy*dy)*(T(i,j+1,t)-2*T(i,j,t)+T(i,j-1,t))+tempDiff+(P*dt)/(Cw*b*b*h*Rho);
               end;
            end;
        end;
    end;
