...
object AGDS extends App {
  ...
  ioData.map { case LoadedData(irises, params) =>
    ...
    val lookingFor = irises(92).copy(weight = 1.0)
    ...
    classify(irises(100), irises, params)
  }.unsafeRunSync()

  def mostSimilar(iris: IrisAGDS, similarities: List[(Int, Double)], irises: List[IrisAGDS]): Unit = {
    val mostSimilar = similarities.sortBy(-_._2).take(6).map(_._1).map(irises(_).show)

    println(s"${iris.show} is most similar to: ")
  ...
  def classify(iris: IrisAGDS, irises: List[IrisAGDS], params: Map[String, Map[Double, List[IrisAGDS]]]) = {
    val lookingFor = iris.copy(id = 1337, weight = 1.0, className = "vz")
